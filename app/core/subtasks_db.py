...
_SUBTASK_COLS = (
    "id, user_id, task_id, title, is_completed, sort_order, created_at, completed_at"
)
    ...
    async with pool.connection() as conn:
        ...
        async with conn.transaction():
            row = await conn.execute(
                """
                INSERT INTO subtask (task_id, title, sort_order, user_id)
                VALUES (%s, %s, %s, %s)
                RETURNING """ + _SUBTASK_COLS,
                (task_id, title, sort_order, user_id),
            )
...
async def get_subtask(
    ...
    """Fetch a single subtask by id scoped to the user, or None."""
    pool = get_pool()
    where_task = " AND task_id = %s" if task_id is not None else ""
    params: list[object] = [subtask_id, user_id]
    ...
    async with pool.connection() as conn:
        conn.row_factory = dict_row
        cur = await conn.execute(
            f"SELECT {_SUBTASK_COLS} FROM subtask WHERE id = %s AND user_id = %s{where_task}",
            params,
        )
        return await cur.fetchone()


async def update_subtask(subtask_id: UUID, user_id: UUID, **fields: object) -> dict | None:
    ...
    if not set_parts:
        ...
        return await get_subtask(subtask_id=subtask_id, user_id=user_id)

    params.extend([subtask_id, user_id])
...
async def complete_subtask(subtask_id: UUID, user_id: UUID) -> dict | None:
...
async def reorder_subtasks(task_id: UUID, user_id: UUID, subtask_ids: list[UUID]) -> dict:
        ...
        async with conn.transaction():
            ...
            for index, subtask_id in enumerate(subtask_ids):
                await conn.execute(
                    ...
                    """,
                    (index, subtask_id, task_id, user_id),
                )
...
async def delete_subtask(subtask_id: UUID, user_id: UUID) -> bool:
    """Delete a subtask. Returns True if a row was removed."""
    pool = get_pool()
    async with pool.connection() as conn:
        async with conn.transaction():
            cur = await conn.execute(
                "DELETE FROM subtask WHERE id = %s AND user_id = %s",
                (subtask_id, user_id),
            )
